...
public class GenericTokenParser {
	...
	public String parse(String text) {
		...
		if (text != null && text.length() > 0) {
			...
			// 这里是循环解析参数，参考GenericTokenParserTest,比如可以解析${first_name} ${initial}
			// ${last_name} reporting.这样的字符串,里面有3个 ${}
			while (start > -1) {
				...
				if (start > 0 && src[start - 1] == '\\') {
					...
					// start指向开始标记的前端, offset指向开始标记的后端
					offset = start + openToken.length();
				} else {
					int end = text.indexOf(closeToken, start); // 获取结束标记的位置
					if (end == -1) {

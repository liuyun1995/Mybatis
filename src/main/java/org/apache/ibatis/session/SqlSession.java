...
/**
 * 这是MyBatis主要的一个类，用来执行SQL，获取映射器，管理事务。 通常情况下，我们在应用程序中使用的Mybatis的API就是这个接口定义的方法。
 */
public interface SqlSession extends Closeable {
	 ...
	 * 
	 * @param <T>
	 *            the returned object type 封装之后的对象类型
	 * @param statement
	 *            sqlID
	...
	/**
	 * Retrieve a list of mapped objects from the statement key and parameter,
	 * within the specified row bounds. 获取多条记录，这个方法容许我们可以传递一些参数，不过这个方法容许我们进行 分页查询。
	 *
	 * 需要注意的是默认情况下，Mybatis为了扩展性，仅仅支持内存分页。也就是会先把
	 * 所有的数据查询出来以后，然后在内存中进行分页。因此在实际的情况中，需要注意 这一点。
	 *
	 ...
	 * {@code ResultHandler}. 获取一条记录,并转交给ResultHandler处理。这个方法容许我们自己定义对
	...
	/**
	 * Retrieve a single row mapped from the statement key and parameter using a
	 * {@code ResultHandler} and {@code RowBounds} 获取一条记录,加上分页,并转交给ResultHandler处理
	 * 
	 * @param statement
	 *            Unique identifier matching the statement to use.
	 * @param rowBounds
	...
	/**
	 * Execute an insert statement. 插入记录。一般情况下这个语句在实际项目中用的并不是太多，而且更多使用带参数的insert函数
	 * 
	 * @param statement
	...
	/**
	 * Retrieves a mapper. 得到映射器 这个巧妙的使用了泛型，使得类型安全 到了MyBatis3，还可以用注解,这样xml都不用写了
	 * 
	 * @param <T>
	 *            the mapper type
	 * @param type
